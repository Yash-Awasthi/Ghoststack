  ...
  it("loads settings from CLAUDE_MEM_DATA_DIR and sends canonical memorySessionIds", async () => {
    ...
    const fetchMock = mock(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url.startsWith("http://localhost:45678/api/search?")) {
        ...
        return new Response(
          JSON.stringify({
            observations: [{ memory_session_id: "memory-a" }, { memory_session_id: "memory-b" }],
            sessions: [{ memory_session_id: "memory-a" }],
            prompts: []
          }),
          { status: 200 }
        );
      ...
      if (url === "http://localhost:45678/api/sdk-sessions/batch") {
        ...
        return new Response(JSON.stringify([{ memory_session_id: "memory-a" }, { memory_session_id: "memory-b" }]), {
          status: 200
        });
      }
