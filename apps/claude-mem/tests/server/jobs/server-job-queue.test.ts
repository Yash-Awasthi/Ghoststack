...
describe("ServerJobQueue", () => {
  ...
  it("error listener absorbs worker errors without throwing", () => {
    ...
    sjq.start(async () => {});
    expect(() => workerState.errorHandlers[0]!(new Error("worker crashed"))).not.toThrow();
  });
  ...
  it("Phase 12 — emits completed/failed/stalled lifecycle events through observe()", () => {
    ...
    sjq.observe({
      onCompleted: (jobId, durationMs) => {
        events.push({ kind: "completed", jobId, arg: durationMs });
      },
      onFailed: (jobId, attempts, reason) => {
        events.push({ kind: "failed", jobId: jobId ?? "?", arg: { attempts, reason } });
      },
      onStalled: (jobId) => {
        events.push({ kind: "stalled", jobId });
      },
      onError: (err) => {
        events.push({ kind: "error", arg: err });
      }
    });
    sjq.start(async () => {});
    ...
    workerState.eventHandlers?.get("failed")?.(
