...
function makeContext(
  overrides: Partial<{ payload: unknown; serverSessionId: string | null }> = {}
...
describe("shared error classification", () => {
  ...
  it("classifyClaudeServerError treats prompt-too-long as unrecoverable", () => {
    expect(classifyClaudeServerError({ status: 400, bodyText: "prompt is too long", cause: "x" }).kind).toBe(
      "unrecoverable"
    );
  });
});
