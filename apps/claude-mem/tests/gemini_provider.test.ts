...
describe("GeminiProvider", () => {
  ...
  beforeEach(() => {
    ...
    getSpy = spyOn(SettingsDefaultsManager, "get").mockImplementation((key: string) => {
      ...
      return (
        SettingsDefaultsManager.getAllDefaults()[
          key as keyof ReturnType<typeof SettingsDefaultsManager.getAllDefaults>
        ] ?? ""
      );
    });
  ...
  it("should throw on rate limit (429) error — no Claude fallback (#2087)", async () => {
    ...
    global.fetch = mock(() =>
      Promise.resolve(new Response("Resource has been exhausted (e.g. check quota).", { status: 429 }))
    );
