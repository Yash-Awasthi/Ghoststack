...
const installSourcePath = join(__dirname, "..", "src", "npx-cli", "commands", "install.ts");
const installSource = readFileSync(installSourcePath, "utf-8");
const codexInstallerSourcePath = join(__dirname, "..", "src", "services", "integrations", "CodexCliInstaller.ts");
const codexInstallerSource = readFileSync(codexInstallerSourcePath, "utf-8");
const syncMarketplaceSourcePath = join(__dirname, "..", "scripts", "sync-marketplace.cjs");
const syncMarketplaceSource = readFileSync(syncMarketplaceSourcePath, "utf-8");
const transcriptConfigSourcePath = join(__dirname, "..", "src", "services", "transcripts", "config.ts");
...
describe("Install Non-TTY Support", () => {
  ...
  describe("non-interactive install path", () => {
    ...
    it("checks Codex CLI marketplace version before registration", () => {
      ...
      expect(installRegion.indexOf("assertCodexMarketplaceSupported()")).toBeLessThan(
        installRegion.indexOf("registerCodexMarketplace(marketplaceRoot)")
      );
    });
    ...
    it("removes legacy Codex AGENTS context only after marketplace registration succeeds", () => {
      ...
      expect(installRegion.indexOf("registerCodexMarketplace(marketplaceRoot)")).toBeLessThan(
        installRegion.indexOf("cleanupLegacyCodexAgentsMdContext()")
      );
    });
