...
describe("server beta postgres observation storage", () => {
  ...
  it("creates observations, searches content, links sources, and preserves generation retry idempotency", async () => {
    ...
    await expect(
      storage.observationSources.listByObservationForScope({
        observationId: observation.id,
        projectId: project.id,
        teamId: project.teamId
      })
  ...
  it("does not mutate scoped observation source, job transition, or job event writes with the wrong scope", async () => {
    ...
    await expect(
      storage.observationSources.addSource({
        observationId: observation.id,
        projectId: other.project.id,
        teamId: other.project.teamId,
        sourceType: "agent_event",
    ...
    await expect(
      storage.observationGenerationJobs.transitionStatus({
        id: eventJob.id,
        projectId: other.project.id,
        teamId: other.project.teamId,
        status: "processing",
    ...
    await expect(
      storage.observationGenerationJobEvents.append({
        generationJobId: eventJob.id,
        projectId: other.project.id,
        teamId: other.project.teamId,
    ...
    await expect(
      storage.observationGenerationJobs.getByIdForScope({
        id: eventJob.id,
        projectId: project.id,
        teamId: project.teamId
      })
    ...
    await expect(
      storage.observationGenerationJobEvents.listByJobForScope({
        generationJobId: eventJob.id,
        projectId: project.id,
        teamId: project.teamId
      })
  ...
  it("exposes scoped getters for auth-visible project resources", async () => {
    ...
    await expect(storage.projects.getByIdForTeam(project.id, project.teamId)).resolves.toMatchObject({
      id: project.id
    });
    await expect(
      storage.sessions.getByIdForScope({
        id: session.id,
        projectId: project.id,
        teamId: project.teamId
      })
    ...
    await expect(
      storage.agentEvents.getByIdForScope({
        id: event.id,
        projectId: project.id,
        teamId: project.teamId
      })
    ...
    await expect(
      storage.observations.getByIdForScope({
        id: observation.id,
        projectId: project.id,
        teamId: project.teamId
      })
  ...
  it("validates server session ownership when creating event generation jobs", async () => {
    ...
    await expect(
      storage.observationGenerationJobs.create({
        projectId: scope.project.id,
        teamId: scope.team.id,
        sourceType: "agent_event",
