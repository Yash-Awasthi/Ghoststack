...
function makeObservationsResponse(
  observations: Array<{ id: number; created_at_epoch: number; type?: string; title?: string }>
) {
  return new Response(
    JSON.stringify({
    ...
    fetchSpy = spyOn(globalThis, "fetch").mockImplementation((url: string | URL | Request) => {
      ...
      if (text.includes("other.md")) {
        return Promise.resolve(
          makeObservationsResponse([{ id: 2, created_at_epoch: future, title: "Other file context" }])
        );
      }
      return Promise.resolve(
