...
describe("ServerBetaClient", () => {
  ...
  it("startSession sends POST /v1/sessions/start with expected payload", async () => {
    installFetch(
      async () =>
        new Response(
          JSON.stringify({
            session: { id: "sess-1", projectId: "p1", teamId: "t1", externalSessionId: "ext", contentSessionId: "ext" }
          }),
  ...
  it("recordEvent sends POST /v1/events with payload", async () => {
    installFetch(
      async () =>
        new Response(JSON.stringify({ event: { id: "e1", projectId: "p1", serverSessionId: null } }), { status: 201 })
    );
    const client = new ServerBetaClient({ serverBaseUrl: "http://localhost:9999", apiKey: "cmem_test" });
  ...
  it("searchObservations sends POST /v1/search with query", async () => {
    installFetch(
      async () =>
        new Response(JSON.stringify({ observations: [{ id: "o1", projectId: "p1", content: "matched" }] }), {
          status: 200
        })
    );
    const client = new ServerBetaClient({ serverBaseUrl: "http://localhost:9999", apiKey: "cmem_test" });
    ...
    installFetch(
      async () =>
        new Response(
          JSON.stringify({
            observations: [
              { id: "o1", projectId: "p1", content: "a" },
              { id: "o2", projectId: "p1", content: "b" }
            ],
            context: "a\n\nb"
          }),
          { status: 200 }
