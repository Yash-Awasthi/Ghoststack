...
describe("plugin/scripts/version-check.js install marker compatibility", () => {
  ...
  beforeEach(() => {
    tempDir = join(tmpdir(), `version-check-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
  ...
  it("emits an upgrade hint for a mismatched legacy plain-text marker", () => {
    ...
    expect(result.status).toBe(0);
    expect(result.stderr).toContain("claude-mem: upgraded to v12.4.4 - run: npx claude-mem@latest install");
  });
});
