...
import { storeObservations, storeObservationsAndMarkComplete } from "../../src/services/sqlite/transactions.js";
...
describe("Transactions Module", () => {
  ...
  ): { memorySessionId: string; sessionDbId: number } {
  ...
  describe("storeObservations", () => {
    ...
    it("should store all observations with same timestamp", () => {
      ...
      const project = "test-project";
      const observations = [createObservationInput({ title: "Obs A" }), createObservationInput({ title: "Obs B" })];
      const fixedTimestamp = 1600000000000;

      const result = storeObservations(db, memorySessionId, project, observations, null, 1, 0, fixedTimestamp);
    ...
    it("should return correct createdAtEpoch", () => {
      ...
      const result = storeObservations(db, memorySessionId, "project", [createObservationInput()], null);
    ...
    it("should apply promptNumber to all observations", () => {
      ...
      const project = "test-project";
      const observations = [createObservationInput({ title: "Obs 1" }), createObservationInput({ title: "Obs 2" })];
      const promptNumber = 5;

      const result = storeObservations(db, memorySessionId, project, observations, null, promptNumber);
  ...
  describe("storeObservationsAndMarkComplete", () => {
    it("should store observations, summary, and remove completed queue message", () => {
      ...
      const result = storeObservationsAndMarkComplete(db, memorySessionId, project, observations, summary, messageId);
    ...
    it("should maintain atomicity - all operations share same timestamp", () => {
      const { memorySessionId, sessionDbId } = createSessionWithMemoryId(
        "content-atomic-ts",
        "atomic-timestamp-session"
      );
      const project = "test-project";
      const observations = [createObservationInput({ title: "Obs 1" }), createObservationInput({ title: "Obs 2" })];
    ...
    it("should handle null summary", () => {
      ...
      const result = storeObservationsAndMarkComplete(db, memorySessionId, project, observations, null, messageId.id);
    ...
    it("should roll back stored observations when the pending message is not completed", () => {
      ...
      expect(() =>
        storeObservationsAndMarkComplete(db, memorySessionId, "test-project", observations, null, 99999)
