...
describe("ChromaSync Vector Sync Integration", () => {
  ...
  describe("Error handling", () => {
    it("should handle connection failures gracefully", async () => {
      ...
      try {
        await sync.syncObservation(1, "session-123", "test", observation, 1, Date.now());
  ...
  describe("Process leak prevention (Issue #761)", () => {
    it("should have transport cleanup in ChromaMcpManager error handlers", async () => {
      const sourceFile = await Bun.file(new URL("../../src/services/sync/ChromaMcpManager.ts", import.meta.url)).text();
