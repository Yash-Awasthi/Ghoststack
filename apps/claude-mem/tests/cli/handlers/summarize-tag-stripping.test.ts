...
describe("summarizeHandler — privacy tag stripping", () => {
  ...
  it("preserves surrounding content when stripping privacy tags", async () => {
    mockExtractedMessage = "Before tag. <private>leak</private> Middle. <private>another</private> After.";
  ...
  it("does not modify content that contains no privacy tags", async () => {
    ...
    const body = postedBody();
    expect(body.last_assistant_message).toBe("Just a normal assistant turn with no privacy markers.");
  });
