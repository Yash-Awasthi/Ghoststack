...
import { readInstallMarker, writeInstallMarker, isInstallCurrent } from "../src/npx-cli/install/setup-runtime";
...
describe("setup-runtime install marker", () => {
  ...
  beforeEach(() => {
    tempDir = join(tmpdir(), `setup-runtime-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
