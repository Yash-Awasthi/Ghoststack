...
function getColumnNames(db: Database, table: string): string[] {
  ...
  return (db.prepare(`PRAGMA table_info(${quotedTable})`).all() as { name: string }[]).map((column) => column.name);
}

function getIndexNames(db: Database, table: string): string[] {
  const quotedTable = `"${table.replace(/"/g, '""')}"`;
  return (db.prepare(`PRAGMA index_list(${quotedTable})`).all() as { name: string }[]).map((index) => index.name);
}
...
describe("PendingMessageStore current schema guardrails", () => {
  ...
  test("SessionStore does not stamp dead-column cleanup when a drop fails", () => {
    ...
    const repairedStore = new SessionStore(db);
    try {
      const version31 = db.prepare("SELECT version FROM schema_versions WHERE version = ?").get(31);
...
describe("PendingMessageStore", () => {
  ...
  test("resetProcessingToPending only affects the specified session", () => {
    ...
    const session1Msg = db.query("SELECT status FROM pending_messages WHERE id = ?").get(session1MessageId) as {
      status: string;
    };
    const session2Msg = db.query("SELECT status FROM pending_messages WHERE id = ?").get(session2MessageId) as {
      status: string;
    };
