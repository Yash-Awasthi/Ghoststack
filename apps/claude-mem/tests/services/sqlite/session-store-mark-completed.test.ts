...
describe("SessionStore.markSessionCompleted", () => {
  ...
  it("sets status to completed and records completed_at timestamps", () => {
    ...
    const row = store.db
      .prepare("SELECT status, completed_at, completed_at_epoch FROM sdk_sessions WHERE id = ?")
  ...
  it("leaves other sessions unaffected", () => {
    ...
    const row2 = store.db.prepare("SELECT status, completed_at FROM sdk_sessions WHERE id = ?").get(id2) as {
      status: string;
  ...
  it("completed_at is a valid ISO timestamp", () => {
    ...
    const row = store.db.prepare("SELECT completed_at FROM sdk_sessions WHERE id = ?").get(id) as {
      completed_at: string;
    };
