...
describe("Schema repair on malformed database", () => {
  it("should repair a database with an orphaned index referencing a non-existent column", () => {
    ...
    try {
      ...
      const indexes = repaired.db
        .prepare("SELECT name FROM sqlite_master WHERE type='index' AND name='idx_observations_content_hash'")
        .all() as { name: string }[];
