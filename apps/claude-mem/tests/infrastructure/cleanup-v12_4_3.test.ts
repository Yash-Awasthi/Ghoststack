...
function seedDatabase(
  dbPath: string,
  opts: { observerSessions: number; stuckCount: number }
): { observerSessionDbIds: number[]; keepSessionDbId: number } {
...
describe("runOneTimeV12_4_3Cleanup", () => {
  ...
  it("purges observer-sessions and stuck pending_messages, writes marker, wipes chroma", () => {
    ...
    const observerCount = (
      verify.prepare("SELECT COUNT(*) AS n FROM sdk_sessions WHERE project = ?").get(OBSERVER_SESSIONS_PROJECT) as {
        n: number;
      }
    ).n;
    const realCount = (
      verify.prepare(`SELECT COUNT(*) AS n FROM sdk_sessions WHERE project = 'real-project'`).get() as { n: number }
    ).n;
