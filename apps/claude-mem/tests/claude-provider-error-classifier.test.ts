...
import { classifyClaudeError, __resetEffortHintLatchForTesting } from "../src/services/worker/ClaudeProvider.js";
...
describe("classifyClaudeError — HTTP 400 handling (#2357)", () => {
  ...
  it('classifies 400 with "effort parameter" body as unrecoverable AND logs an SDK warn once', () => {
    const sdkErr = Object.assign(new Error("This model does not support the effort parameter."), { status: 400 });
  ...
  it("classifies 400 without effort body as unrecoverable WITHOUT firing the effort hint", () => {
    const sdkErr = Object.assign(new Error("some other 400 error"), { status: 400 });
