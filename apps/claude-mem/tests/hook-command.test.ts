...
describe("isNonBlockingHookInputError", () => {
  it("classifies missing transcript paths as non-blocking hook input errors", () => {
    const error = new Error("Transcript path missing or file does not exist: /tmp/missing-session.jsonl");
  ...
  it("classifies missing transcript-path errors without file-existence text", () => {
    expect(isNonBlockingHookInputError(new Error("Transcript path missing: /tmp/missing-session.jsonl"))).toBe(true);
  });

  it("classifies nonexistent transcript-path errors without missing text", () => {
    expect(isNonBlockingHookInputError(new Error("Transcript path does not exist: /tmp/missing-session.jsonl"))).toBe(
      true
    );
  });
