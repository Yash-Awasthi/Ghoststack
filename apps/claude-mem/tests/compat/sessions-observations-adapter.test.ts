...
describe("Phase 9 compat adapters", () => {
  ...
  it("POST /api/sessions/summarize ends server_session and enqueues summary job (legacy response shape)", async () => {
    ...
    const sessionRows = await client.query(`SELECT ended_at FROM server_sessions WHERE id = $1`, [
    ...
    const outboxRows = await client.query(`SELECT source_type FROM observation_generation_jobs WHERE id = $1`, [
