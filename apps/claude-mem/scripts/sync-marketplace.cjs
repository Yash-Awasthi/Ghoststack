...
function getGitignoreExcludes(basePath) {
  ...
  return lines
    .map((line) => line.trim())
    .filter((line) => line && !line.startsWith("#") && !line.startsWith("!") && !syncManagedFiles.has(line))
...
try {
  ...
  execSync("cd ~/.claude/plugins/marketplaces/thedotmack/ && bun install", { stdio: "inherit" });
  ...
  execSync(`rsync -av --delete --exclude=.git ${pluginGitignoreExcludes} plugin/ "${CACHE_VERSION_PATH}/"`, {
    stdio: "inherit"
  });
  ...
  if (installedMismatch && installedMismatch.installedVersion !== version) {
    ...
    execSync(`rsync -av --delete --exclude=.git ${pluginGitignoreExcludes} plugin/ "${INSTALLED_CACHE_PATH}/"`, {
      stdio: "inherit"
    });
  ...
  const workerPort = parseWorkerPort(process.env.CLAUDE_MEM_WORKER_PORT) ?? settingsPort ?? defaultPort;
  ...
  req.on("error", () => {
    console.log(
      "\x1b[33m%s\x1b[0m",
      `ℹ No worker reachable on port ${workerPort}; the next worker:restart step will start one.`
    );
  });
  req.on("timeout", () => {
    req.destroy();
