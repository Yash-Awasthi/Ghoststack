...
`);
  ...
  const counts = db
    .prepare("SELECT status, COUNT(*) as count FROM pending_messages GROUP BY status")
    .all() as StatusRow[];
  ...
  const remaining = (db.prepare("SELECT COUNT(*) as count FROM pending_messages").get() as CountRow).count;
