...
function detectAntiPatterns(filePath: string, projectRoot: string): AntiPattern[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const hasOverride =
      trimmed.includes("[ANTI-PATTERN IGNORED]") || (i > 0 && lines[i - 1].includes("[ANTI-PATTERN IGNORED]"));
      ...
      if (pattern.test(trimmed)) {
        if (hasOverride && overrideReason) {
          ...
          antiPatterns.push({
            ...
            severity: "ISSUE",
            description:
              "Logging only error.message HIDES the stack trace, error type, and all properties. ALWAYS pass the full error object - you need the complete picture, not a summary.",
            code: trimmed
          });
        }
      }
    ...
    const multipleIncludes = trimmed.match(
      /(?:error(?:Message|\.message)|(?:err|e)\.message).*\.includes.*\|\|.*\.includes/i
    );
    if (multipleIncludes) {
      if (hasOverride && overrideReason) {
        antiPatterns.push({
        ...
        antiPatterns.push({
          ...
          severity: "ISSUE",
          description:
            "Multiple string checks on error message to guess error type. STOP GUESSING. Log the FULL error object. We don't care what the library throws - we care about SEEING the error when it happens.",
          code: trimmed
        });
      }
    }
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (promiseCatchMatch && !emptyPromiseCatch) {
      ...
      const hasLogging =
        catchBody.match(/logger\.(error|warn|debug|info|failure)/) || catchBody.match(/console\.(error|warn)/);

      if (!hasLogging && lookAhead > 0) {
        // Only flag if it's actually a multi-line handler
        antiPatterns.push({
          file: relPath,
          line: i + 1,
...
function analyzeTryCatchBlock(
  ...
  const hasTypeCheck =
    catchContent.match(/instanceof\s+Error/) ||
