...
function main() {
  ...
  console.log(
    `Time window: ${windowMode} (${batchWindowSeconds === 0 ? "ignore time" : batchWindowSeconds + " seconds"})`
  );
  console.log("");
  console.log("Options:");
  ...
  const db = dryRun ? new Database(DB_PATH, { readonly: true }) : new Database(DB_PATH);
