...
async function buildViewer() {
  ...
  try {
    ...
    const htmlTemplate = fs.readFileSync(path.join(rootDir, "src/ui/viewer-template.html"), "utf-8");
    fs.writeFileSync(path.join(rootDir, "plugin/ui/viewer.html"), htmlTemplate);
    ...
    if (fs.existsSync(fontsDir)) {
      ...
      for (const file of fontFiles) {
        fs.copyFileSync(path.join(fontsDir, file), path.join(outputFontsDir, file));
      }
    }
    ...
    const iconFiles = fs
      .readdirSync(srcUiDir)
      .filter((file) => file.startsWith("icon-thick-") && file.endsWith(".svg"));
    for (const file of iconFiles) {
      fs.copyFileSync(path.join(srcUiDir, file), path.join(outputUiDir, file));
    }
