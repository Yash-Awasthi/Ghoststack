...
function classify(cwd: string): Classification {
  ...
  const parentRepoDir = commonDir.endsWith("/.git") ? dirname(commonDir) : commonDir.replace(/\.git$/, "");
...
function main() {
  ...
  const skipped = [...byCwd.entries()].filter(([, c]) => c.kind === "skip") as Array<
    [string, Extract<Classification, { kind: "skip" }>]
  >;
  if (skipped.length) {
    console.log("\nSkipped cwds:");
