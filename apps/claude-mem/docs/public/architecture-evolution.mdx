...
// v4: Graceful
async function sessionEnd(sessionId: string) {
  await db.run("UPDATE sdk_sessions SET completed_at = ? WHERE id = ?", [Date.now(), sessionId]);
}
```
...
```typescript
// ❌ Vulnerable
const results = db.query(`SELECT * FROM observations_fts WHERE observations_fts MATCH '${userQuery}'`);
```
...
const results = db.query(`SELECT * FROM observations WHERE text LIKE '%${query}%'`);
```
...
```typescript
// FTS5 virtual table
const results = db.query(`SELECT * FROM observations_fts WHERE observations_fts MATCH ?`, [query]);
```
...
_This architecture evolution reflects hundreds of hours of experimentation, dozens of dead ends, and the invaluable experience of real-world usage. v5 is the architecture that emerged from understanding what actually works - and making it visible to users._
