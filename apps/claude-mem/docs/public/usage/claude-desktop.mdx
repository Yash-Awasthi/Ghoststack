...
<Note>**Availability:** Claude-mem MCP tools work with Claude Desktop on macOS and Windows.</Note>
...
<Tabs>
  <Tab title="macOS">```bash # View Claude Desktop logs tail -f ~/Library/Logs/Claude/claude.log ```</Tab>
  <Tab title="Windows">Check `%APPDATA%\Claude\logs\`</Tab>
</Tabs>
