...
**Tag Stripping Scope**: The implementation strips tags from the _serialized JSON representations_ of tool inputs and tool responses, not from the original user prompt text in the conversation UI. The user prompt text you type is stored in a separate table (user_prompts) where tags are also stripped before storage.
