...
export class ServerV1PostgresRoutes implements RouteHandler {
  ...
  setupRoutes(app: Application): void {
    ...
    app.post(
      ...
      this.asyncHandler(async (req, res) => {
        ...
        res.status(201).json({
          event: serializeEvent(event),
          ...(outbox ? { generationJob: serializeGenerationJob(outbox, enqueueState) } : {})
        });
      })
    );
      ...
      this.asyncHandler(async (req, res) => {
        ...
        if (wait) {
          ...
          res.status(201).json({
            events: resolved.map(({ event, outbox, timedOut }, index) => ({
              event: serializeEvent(event),
              generationJob: outbox ? serializeJobStatusResponse(outbox, enqueueResults[index]!) : null,
        ...
        res.status(201).json({
          events: inserted.map(({ event, outbox }, index) => ({
            event: serializeEvent(event),
            ...(outbox ? { generationJob: serializeGenerationJob(outbox, enqueueResults[index]!) } : {})
          }))
        });
      })
    );
    ...
    app.get(
      ...
      this.asyncHandler(async (req, res) => {
        ...
        const result = await this.options.pool.query(`SELECT * FROM agent_events WHERE id = $1 AND team_id = $2`, [
          id,
          teamId
        ]);
    ...
    app.get(
      ...
      this.asyncHandler(async (req, res) => {
        ...
        const includePayload = includeRaw
          .split(",")
          .map((p) => p.trim())
          .includes("payload");
        const callerScopes = req.authContext?.scopes ?? [];
        const isAdmin =
          callerScopes.includes("*") || callerScopes.includes("admin") || callerScopes.includes("memories:admin");
        ...
        async (req, res, body) => {
          ...
          try {
            ...
            try {
              ...
              if (body.externalSessionId && (error as { code?: string } | null)?.code === "23505") {
    ...
    app.post(
      ...
      this.asyncHandler(async (req, res) => {
        ...
        res.status(200).json({
          ...
          ...(summaryOutbox ? { generationJob: serializeGenerationJob(summaryOutbox, enqueueState) } : {})
        });
      })
    );
  ...
  private toAgentEventInput(
    ...
    return {
      ...
      sourceAdapter,
      sourceEventId:
        typeof (body as Record<string, unknown>).sourceEventId === "string"
      ...
      try {
        ...
        if (bullmqJobId) {
          ...
          try {
            ...
            /* terminal slot may be missing — ok */
          }
...
function serializeJobListEntry(row: JobListRow, options: { includePayload?: boolean } = {}): Record<string, unknown> {
...
function preValidateBatch(req: Request, events: { projectId: string }[]): BatchPreValidationFailure | null {
...
function serializeGenerationJobStatus(job: PostgresObservationGenerationJob): Record<string, unknown> {
  return {
    id: job.id,
    projectId: job.projectId,
    teamId: job.teamId,
