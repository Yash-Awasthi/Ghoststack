...
export class ServerV1Routes implements RouteHandler {
  ...
  setupRoutes(app: Application): void {
    ...
    app.post(
      "/v1/projects",
      writeAuth,
      this.handleCreate(CreateProjectSchema, (req, res, body) => {
    ...
    app.post(
      "/v1/sessions/start",
      writeAuth,
      this.handleCreate(CreateServerSessionSchema, (req, res, body) => {
    ...
    app.post(
      "/v1/events",
      writeAuth,
      this.handleCreate(CreateAgentEventSchema, (req, res, body) => {
        if (!this.ensureProjectAllowed(req, res, body.projectId)) return;
    ...
    app.post(
      "/v1/events/batch",
      writeAuth,
      this.handleCreate(z.array(CreateAgentEventSchema).min(1).max(500), (req, res, body) => {
    ...
    app.post(
      "/v1/memories",
      writeAuth,
      this.handleCreate(CreateMemoryItemSchema, (req, res, body) => {
        if (!this.ensureProjectAllowed(req, res, body.projectId)) return;
    ...
    app.patch(
      "/v1/memories/:id",
      writeAuth,
      this.handleCreate(CreateMemoryItemSchema.partial(), (req, res, body) => {
        const id = this.routeParam(req.params.id);
    ...
    app.post(
      ...
      this.handleCreate(
        ...
        (req, res, body) => {
          if (!this.ensureProjectAllowed(req, res, body.projectId)) return;
          const memories = new MemoryItemsRepository(this.options.getDatabase()).search(
            body.projectId,
            body.query,
            body.limit ?? 20
          );
      ...
      this.handleCreate(
        ...
        (req, res, body) => {
          ...
          res.json({
            memories,
            context: memories
              .map((memory) => memory.narrative ?? memory.text ?? memory.title)
              .filter(Boolean)
              .join("\n\n")
          });
        }
