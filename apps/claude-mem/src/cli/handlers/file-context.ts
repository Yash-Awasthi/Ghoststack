...
export const fileContextHandler: EventHandler = {
  async execute(input: NormalizedHookInput): Promise<HookResult> {
    ...
    const filePaths = Array.isArray(toolInput?.filePaths)
      ? (toolInput.filePaths as unknown[])
          .filter((p): p is string => typeof p === "string")
          .slice(0, MAX_FILE_CONTEXT_PATHS)
      : [];
...
async function buildFileContextTimeline(input: NormalizedHookInput, filePath: string): Promise<string | null> {
  let fileMtimeMs = 0;
  try {
    const statPath = path.isAbsolute(filePath) ? filePath : path.resolve(input.cwd || process.cwd(), filePath);
    ...
    logger.debug("HOOK", "File stat failed, proceeding with gate", {
