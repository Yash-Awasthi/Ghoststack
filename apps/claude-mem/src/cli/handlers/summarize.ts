  ...
  async execute(input: NormalizedHookInput): Promise<HookResult> {
    ...
    if (input.lastAssistantMessage !== undefined) {
      ...
      try {
        ...
        logger.warn(
          "HOOK",
          `Stop hook: failed to extract last assistant message for session ${sessionId}: ${err instanceof Error ? err.message : err}`
        );
        return { continue: true, suppressOutput: true, exitCode: HOOK_EXIT_CODES.SUCCESS };
      }
    }
    ...
    const queueResult = await executeWithWorkerFallback<{ status?: string }>("/api/sessions/summarize", "POST", {
      contentSessionId: sessionId,
      last_assistant_message: lastAssistantMessage,
      platformSource
    });
