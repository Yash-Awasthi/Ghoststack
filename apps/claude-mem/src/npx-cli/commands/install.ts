    ...
    case "cursor": {
      return {
        ...
        task: async (message) => {
          ...
          const { installCursorHooks, configureCursorMcp } =
            await import("../../services/integrations/CursorHooksInstaller.js");
...
function applyClaudeCodePathSetupIfNeeded(): void {
  ...
  const exportLine = shell === "fish" ? `set -gx PATH ${claudeBinDir} $PATH` : `export PATH="${binPathLiteral}:$PATH"`;
...
async function maybeBootstrapServerBetaApiKey(): Promise<void> {
  ...
  try {
    const { bootstrapServerBetaApiKey, persistServerBetaSettings } =
    ...
    log.info(
      `Provisioned local hook API key (project=${result.projectId.slice(0, 8)}…). ` + "Settings saved with mode 0600."
    );
  } catch (error: unknown) {
    log.warn(
...
async function promptProvider(options: InstallOptions): Promise<ProviderId> {
  ...
  if (!isInteractive) {
    if (options.provider) {
      ...
      log.warn(
        `Provider=${options.provider} requested non-interactively. API key prompt skipped — set CLAUDE_MEM_${options.provider.toUpperCase()}_API_KEY and CLAUDE_MEM_PROVIDER in settings.json or env manually if not already set.`
      );
  ...
  const runClaudeAuthFlow = async (): Promise<void> => {
    ...
    const initialAccessMode: ClaudeAccessMode = resolvedAuthMethod === "subscription" ? "subscription" : "api-key";
  ...
  const keyEnvName = selectedProvider === "gemini" ? "CLAUDE_MEM_GEMINI_API_KEY" : "CLAUDE_MEM_OPENROUTER_API_KEY";
...
async function promptClaudeModel(options: InstallOptions): Promise<void> {
  const allowed = new Set(["claude-haiku-4-5-20251001", "claude-sonnet-4-6", "claude-opus-4-7"]);
  const allowCustomModel = resolveClaudeAuthMethod() === "gateway";

  if (options.model && !allowCustomModel) {
    if (!allowed.has(options.model)) {
      throw new Error(`Unknown Claude model: ${options.model}. Allowed: ${[...allowed].join(", ")}`);
    }
...
export async function runInstallCommand(options: InstallOptions = {}): Promise<void> {
      ...
      try {
        ...
        if (shutdownSpinner) {
          shutdownSpinner.stop(
            result.workerWasRunning ? "Stopped running worker before overwrite." : "No worker running — proceeding."
          );
  ...
  await runTasks([
    {
      ...
      task: async (message) => {
        if (autoStartSkipped) {
          return isInteractive ? `Skipped (--no-auto-start)` : `Skipped (non-TTY)`;
        }
  ...
  const runtimeStartCommand =
    selectedRuntime === "server-beta" ? "npx claude-mem server start" : "npx claude-mem start";
