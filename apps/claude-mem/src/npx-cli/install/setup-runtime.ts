...
const LEGACY_VERSION_MARKER_RE = /^v?\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?(?:\+[0-9A-Za-z.-]+)?$/;
...
function installBun(): void {
  try {
    ...
    if (!isBunInstalled()) {
      throw new Error("Bun installation completed but binary not found. Please restart your terminal and try again.");
    }
  } catch (error) {
...
function installUv(): void {
  try {
    ...
    if (!isUvInstalled()) {
      throw new Error("uv installation completed but binary not found. Please restart your terminal and try again.");
    }
  } catch (error) {
    const manualInstructions = IS_WINDOWS
