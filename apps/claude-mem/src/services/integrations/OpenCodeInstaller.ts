...
export async function syncContextToAgentsMd(port: number, project: string): Promise<void> {
  try {
...
async function fetchRealContextFromWorker(): Promise<string | null> {
  ...
  const contextResponse = await fetch(`http://127.0.0.1:${workerPort}/api/context/inject?project=opencode`);
...
async function fetchAndInjectOpenCodeContext(port: number, project: string): Promise<void> {
  const response = await fetch(`http://127.0.0.1:${port}/api/context/inject?project=${encodeURIComponent(project)}`);
...
function writeOrRemoveCleanedAgentsMd(agentsMdPath: string, trimmedContent: string): void {
  if (trimmedContent.length === 0 || trimmedContent === "# Claude-Mem Memory Context") {
