...
function assertCodexMarketplaceSupported(): void {
  ...
  if (result.status !== 0) {
    console.warn(
      `  Could not determine Codex CLI version. Continuing; plugin marketplace support requires ${MIN_CODEX_MARKETPLACE_VERSION} or newer.${output ? `\n${output}` : ""}`
    );
    return;
  }
  ...
  if (minimumVersion && compareSemver(version, minimumVersion) < 0) {
    throw new Error(
      `Codex CLI ${version.join(".")} is too old for plugin marketplace support. Update Codex CLI to ${MIN_CODEX_MARKETPLACE_VERSION} or newer, then run: npx claude-mem@latest install`
    );
  }
}
...
function isLegacyCodexAgentsContext(context: Record<string, unknown>): boolean {
  ...
  const hasLegacyUpdateOn =
    Array.isArray(updateOn) &&
    updateOn.length === 2 &&
  ...
  return typeof context.path === "string" && path.resolve(expandHome(context.path)) === CODEX_AGENTS_MD_PATH;
}
...
export async function installCodexCli(marketplaceRootOverride?: string): Promise<number> {
  ...
  try {
    ...
    if (!cleanupLegacyCodexTranscriptAgentsContext()) {
      console.warn(
        `  Native Codex hooks registered, but failed to disable legacy transcript AGENTS.md context in ${CODEX_TRANSCRIPT_WATCH_CONFIG_PATH}.`
      );
    }
