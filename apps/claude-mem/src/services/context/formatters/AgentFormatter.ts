import type { ContextConfig, Observation, SessionSummary, TokenEconomics, PriorMessages } from "../types.js";
...
export function renderAgentContextEconomics(economics: TokenEconomics, config: ContextConfig): string[] {
  const output: string[] = [];
...
export function renderAgentTableRow(obs: Observation, timeDisplay: string, _config: ContextConfig): string {
...
export function renderAgentSummaryItem(
  ...
  return [`S${summary.id} ${summary.request || "Session started"} (${formattedTime})`];
}
...
export function renderAgentFooter(totalDiscoveryTokens: number, totalReadTokens: number): string[] {
  ...
  return ["", `Access ${workTokensK}k tokens of past work via get_observations([IDs]) or mem-search skill.`];
}
