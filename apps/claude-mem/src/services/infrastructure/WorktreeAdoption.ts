...
function listWorktrees(mainRepo: string): WorktreeEntry[] {
  ...
  for (const line of raw.split("\n")) {
    if (line.startsWith("worktree ")) {
      ...
      current.branch = refName.startsWith("refs/heads/") ? refName.slice("refs/heads/".length) : refName;
...
function listMergedBranches(mainRepo: string): Set<string> {
  const raw = gitCapture(mainRepo, ["branch", "--merged", "HEAD", "--format=%(refname:short)"]);
...
export async function adoptMergedWorktrees(
  opts: {
    repoPath?: string;
  ...
  result.mergedBranches = targets.map((t) => t.branch).filter((b): b is string => b !== null);
  ...
  try {
    ...
    const obsColumns = db.prepare("PRAGMA table_info(observations)").all() as ColumnInfo[];
    const sumColumns = db.prepare("PRAGMA table_info(session_summaries)").all() as ColumnInfo[];
    ...
    const adoptWorktreeInTransaction = (wt: WorktreeEntry) => {
      ...
      const rows = selectObsForPatch.all(worktreeProject, parentProject) as Array<{ id: number }>;
  ...
  if (!dryRun && adoptedSqliteIds.length > 0) {
    ...
    try {
      ...
      if (err instanceof Error) {
        ...
        logger.error("SYSTEM", "Worktree adoption Chroma patch failed (SQL already committed)", {
          parentProject,
          sqliteIdCount: adoptedSqliteIds.length,
          error: String(err)
        });
      }
...
export async function adoptMergedWorktreesForAllKnownRepos(
  opts: {
    dataDirectory?: string;
    dryRun?: boolean;
  } = {}
  ...
  try {
    ...
    const hasPending = db
      .prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='pending_messages'")
  ...
  for (const repoPath of uniqueParents) {
    try {
      ...
      logger.warn("SYSTEM", "Worktree adoption failed for parent repo (continuing)", {
        repoPath,
