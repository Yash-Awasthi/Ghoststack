...
function scanCleanupCounts(dbPath: string): CleanupCounts {
  ...
  try {
    counts.observerSessions = (
      db.prepare(`SELECT COUNT(*) AS n FROM sdk_sessions WHERE project = ?`).get(OBSERVER_SESSIONS_PROJECT) as {
        n: number;
      }
    ).n;
    counts.observerCascadeRows =
      (
        db
          ...
          .get(OBSERVER_SESSIONS_PROJECT) as { n: number }
      ).n +
      (
        db
          .prepare(
            `SELECT COUNT(*) AS n FROM observations WHERE memory_session_id IN (SELECT memory_session_id FROM sdk_sessions WHERE project = ? AND memory_session_id IS NOT NULL)`
          )
          .get(OBSERVER_SESSIONS_PROJECT) as { n: number }
      ).n +
      (
        db
          .prepare(
            `SELECT COUNT(*) AS n FROM session_summaries WHERE memory_session_id IN (SELECT memory_session_id FROM sdk_sessions WHERE project = ? AND memory_session_id IS NOT NULL)`
          )
          .get(OBSERVER_SESSIONS_PROJECT) as { n: number }
      ).n;
    counts.stuckPendingMessages = (
      db
        .prepare(
        ...
        .get(STUCK_PENDING_THRESHOLD) as { n: number }
    ).n;
  } finally {
    db.close();
  }
...
function executeCleanup(dbPath: string, effectiveDataDir: string, markerPath: string): void {
  ...
  try {
    ...
    if (free < required) {
      logger.error("SYSTEM", "Insufficient disk for v12.4.3 backup; skipping cleanup (will retry on next startup)", {
        dbSize,
        free,
        required
      });
  ...
  if (vacuumFailed) {
    ...
    try {
      ...
      logger.info("SYSTEM", "v12.4.3 backup created via copyFileSync (incl. -wal/-shm if present)", {
        backupPath,
        dbSize
      });
      ...
      logger.error(
        "SYSTEM",
        "v12.4.3 backup failed via both VACUUM INTO and copyFileSync; aborting cleanup",
        {},
        copyError
      );
      return;
