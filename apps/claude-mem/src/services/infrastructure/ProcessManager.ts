...
export function parseElapsedTime(etime: string): number {
  ...
  if (dayMatch) {
    totalMinutes = parseInt(dayMatch[1], 10) * 24 * 60 + parseInt(dayMatch[2], 10) * 60 + parseInt(dayMatch[3], 10);
    return totalMinutes;
  }
...
type CwdClassification = { kind: "main"; project: string } | { kind: "worktree"; project: string } | { kind: "skip" };
...
function classifyCwdForRemap(cwd: string): CwdClassification {
  ...
  const parentRepoDir = commonDir.endsWith("/.git") ? path.dirname(commonDir) : commonDir.replace(/\.git$/, "");
...
export function runOneTimeCwdRemap(dataDirectory?: string): void {
  ...
  try {
    ...
    if (err instanceof Error) {
      ...
      logger.error(
        "SYSTEM",
        "cwd-remap failed, marker not written (will retry on next startup)",
        {},
...
export function spawnDaemon(
  ...
  const args = useSetsid ? [runtimePath, scriptPath, "--daemon"] : [scriptPath, "--daemon"];
