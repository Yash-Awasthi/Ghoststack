...
import { createServerApiKey, listServerApiKeys, revokeServerApiKey } from "../server/auth/api-key-service.js";
import { ServerV1Routes } from "../server/routes/v1/ServerV1Routes.js";

import { updateCursorContextForProject, handleCursorCommand } from "./integrations/CursorHooksInstaller.js";
import { handleGeminiCliCommand } from "./integrations/GeminiCliHooksInstaller.js";
...
export class WorkerService implements WorkerRef {
  ...
  constructor() {
    ...
    this.server = new Server({
      ...
      getQueueHealth: () => (this.sessionManager.isBullMqQueueEnabled() ? this.sessionManager.getQueueHealth() : null),
  ...
  private registerRoutes(): void {
    ...
    attachIngestGeneratorStarter((sessionDbId, source) => sessionRoutes.ensureGeneratorRunning(sessionDbId, source));
    ...
    try {
      ...
      adoptMergedWorktreesForAllKnownRepos({})
        ...
        .catch((err) => {
          logger.error(
            "WORKER",
            "Worktree adoption failed (background)",
            {},
      ...
      const sweepResult = this.dbManager
        .getSessionStore()
        .db.prepare(
          `
      ...
      if (this.chromaMcpManager) {
        ChromaSync.backfillAllProjects(this.dbManager.getSessionStore())
          .then(() => {
  ...
  private async runMcpSelfCheck(mcpServerPath: string): Promise<void> {
    try {
      ...
      const timeoutPromise = new Promise<never>((_, reject) => {
        setTimeout(() => reject(new Error("MCP connection timeout")), MCP_INIT_TIMEOUT_MS);
      });
  ...
  private async startTranscriptWatcher(
    ...
    try {
      ...
      if (error instanceof Error) {
        logger.error(
          "WORKER",
          "Failed to start transcript watcher (continuing without transcript ingestion)",
          {
            configPath: resolvedConfigPath
          },
          error
        ...
        logger.error(
          "WORKER",
          "Failed to start transcript watcher with non-Error (continuing without transcript ingestion)",
          {
            configPath: resolvedConfigPath
          },
    ...
    session.generatorPromise = agent
      ...
      .catch(async (error: unknown) => {
        ...
        if (dispatchKind === "unrecoverable" || dispatchKind === "auth_invalid" || dispatchKind === "quota_exhausted") {
          ...
          const logLabel =
            dispatchKind === "auth_invalid"
              ? "auth invalid"
              : dispatchKind === "quota_exhausted"
                ? "quota exhausted"
                : "unrecoverable";
        ...
        if (staleResumePatterns.some((p) => errorMessage.includes(p)) && session.memorySessionId) {
  ...
  private isSessionTerminatedError(error: unknown): boolean {
    ...
    return WorkerService.SESSION_TERMINATED_PATTERNS.some((pattern) => normalized.includes(pattern));
  }
    ...
    if (isGeminiAvailable()) {
      try {
        ...
        if (e instanceof Error) {
          ...
          logger.error(
            "WORKER",
            "Gemini fallback failed with non-Error",
            { sessionId: sessionDbId },
            new Error(String(e))
          );
        }
    ...
    if (isOpenRouterAvailable()) {
      try {
        ...
        if (e instanceof Error) {
          ...
          logger.error(
            "WORKER",
            "Fallback OpenRouter failed with non-Error, will abandon messages",
            { sessionId: sessionDbId },
            new Error(String(e))
          );
        }
...
function printServerCommandHelp(): never {
  ...
  console.error(
    "Commands: start, stop, restart, status, logs, doctor, migrate, export, import, api-key create|list|revoke"
  );
  process.exit(1);
}
...
async function printQueueStatusIfBullMq(port: number): Promise<void> {
  ...
  try {
    ...
    console.log(
      `  Queue: BullMQ Redis ${redis.status ?? "unknown"} at ${target} [${redis.mode ?? "external"}, prefix=${redis.prefix ?? "claude_mem"}]${suffix}`
    );
  } catch (error) {
