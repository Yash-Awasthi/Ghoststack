...
export function classifyOpenRouterError(input: {
  ...
  if (status === 429) {
    return new ClassifiedProviderError("OpenRouter rate limit (429)", {
      kind: "rate_limit",
      cause: input.cause,
  ...
  if (status === 401 || status === 403) {
    return new ClassifiedProviderError(`OpenRouter auth error (status ${status})`, {
      kind: "auth_invalid",
      cause: input.cause
    });
  }
  ...
  if (status === 400 || status === 404) {
    return new ClassifiedProviderError(`OpenRouter bad request (status ${status})`, {
      kind: "unrecoverable",
      cause: input.cause
    });
  }
  ...
  if (status !== undefined && status >= 500 && status < 600) {
    return new ClassifiedProviderError(`OpenRouter upstream error (status ${status})`, {
      kind: "transient",
      cause: input.cause
    });
  }
  ...
  return new ClassifiedProviderError(`OpenRouter API error: ${status}${body ? ` - ${body.substring(0, 200)}` : ""}`, {
    kind: "unrecoverable",
    cause: input.cause
  });
}
...
export class OpenRouterProvider {
  ...
  private prepareMessageMetadata(
    ...
    message: { agentId?: string | null; agentType?: string | null }
  ): void {
  ...
  private truncateHistory(history: ConversationMessage[]): ConversationMessage[] {
    ...
    const MAX_CONTEXT_MESSAGES =
      parseInt(settings.CLAUDE_MEM_OPENROUTER_MAX_CONTEXT_MESSAGES) || DEFAULT_MAX_CONTEXT_MESSAGES;
