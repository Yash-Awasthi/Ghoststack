...
import { globalRateLimitStore, shouldAbortForQuota, type RateLimitInfo } from "./RateLimitStore.js";
...
export function classifyClaudeError(err: unknown): ClassifiedProviderError {
  ...
  if (errAny.name === "OverloadedError" || errAny.status === 529 || errAny.error?.type === "overloaded_error") {
  ...
  if (errAny.status === 400) {
    ...
    return new ClassifiedProviderError(message || "Anthropic bad request (status 400)", {
      kind: "unrecoverable",
      cause: err
    });
  }
  ...
  async startSession(session: ActiveSession, worker?: WorkerRef): Promise<void> {
    ...
    try {
      for await (const message of queryResult) {
        ...
        if ((message as any)?.type === "system" && (message as any)?.subtype === "rate_limit") {
