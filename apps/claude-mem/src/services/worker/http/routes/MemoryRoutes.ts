...
export class MemoryRoutes extends BaseRouteHandler {
  ...
  private handleSaveMemory = this.wrapHandler(async (req: Request, res: Response): Promise<void> => {
    ...
    const explicitProject = typeof project === "string" && project.trim() ? project.trim() : undefined;
    const metadataProject =
      typeof metadata?.project === "string" && metadata.project.trim() ? metadata.project.trim() : undefined;
