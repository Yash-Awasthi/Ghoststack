...
export class SessionRoutes extends BaseRouteHandler {
    ...
    session.generatorPromise = agent
      ...
      .catch(async (error) => {
        ...
        try {
          ...
          logger.error(
            "HTTP",
            "Failed to reset processing messages after generator error",
            {
              sessionId: session.sessionDbId
            },
            normalizedDbError
  ...
  private static readonly sessionInitByClaudeIdSchema = z
    .object({
  ...
  private static readonly observationsByClaudeIdSchema = z
    .object({
      contentSessionId: z.string().min(1),
  ...
  private static readonly summarizeByClaudeIdSchema = z
    .object({
      contentSessionId: z.string().min(1),
  ...
  private handleSessionInitByClaudeId = this.wrapHandler(async (req: Request, res: Response): Promise<void> => {
    ...
    if (rawPrompt && isInternalProtocolPayload(rawPrompt)) {
      logger.debug("HTTP", "session-init: skipping internal protocol payload before session creation", {
        contentSessionId
      });
  ...
  private static readonly SIMPLE_TOOLS = new Set(["Read", "Glob", "Grep", "LS", "ListMcpResourcesTool"]);
  ...
  private async applyTierRouting(
    ...
    const allSimple = pending.every(
      (m) => m.message_type === "observation" && m.tool_name && SessionRoutes.SIMPLE_TOOLS.has(m.tool_name)
    );
