...
export function classifyGeminiError(input: {
  ...
  if (lower.includes("quota exceeded") || lower.includes("resource_exhausted")) {
    return new ClassifiedProviderError(`Gemini quota exhausted${status !== undefined ? ` (status ${status})` : ""}`, {
  ...
  if (status === 429) {
    return new ClassifiedProviderError("Gemini rate limit (429)", {
      kind: "rate_limit",
      cause: input.cause,
  ...
  if (status === 401 || status === 403) {
    ...
    if (lower.includes("api key not valid") || lower.includes("api_key_invalid") || lower.includes("api key expired")) {
      return new ClassifiedProviderError(`Gemini auth invalid (status ${status})`, {
        kind: "auth_invalid",
        cause: input.cause
      });
    }
    return new ClassifiedProviderError(`Gemini auth error (status ${status})`, {
      kind: "auth_invalid",
      cause: input.cause
    });
  }
  ...
  if (status === 400) {
    return new ClassifiedProviderError(`Gemini bad request (status 400)`, {
      kind: "unrecoverable",
      cause: input.cause
    });
  }
  ...
  if (status !== undefined && status >= 500 && status < 600) {
    return new ClassifiedProviderError(`Gemini upstream error (status ${status})`, {
      kind: "transient",
      cause: input.cause
    });
  }
  ...
  return new ClassifiedProviderError(`Gemini API error: ${status}${body ? ` - ${body.substring(0, 200)}` : ""}`, {
    kind: "unrecoverable",
    cause: input.cause
  });
}
...
export class GeminiProvider {
  ...
  async startSession(session: ActiveSession, worker?: WorkerRef): Promise<void> {
    ...
    try {
      ...
      if (error instanceof Error) {
        ...
        logger.error(
          "SDK",
          "Gemini init query failed with non-Error",
    ...
    try {
      ...
      if (error instanceof Error) {
        ...
        logger.error(
          "SDK",
          "Gemini message loop failed with non-Error",
          { sessionId: session.sessionDbId, model },
          new Error(String(error))
        );
      }
  ...
  private truncateHistory(history: ConversationMessage[]): ConversationMessage[] {
    ...
    const MAX_CONTEXT_MESSAGES =
      parseInt(settings.CLAUDE_MEM_GEMINI_MAX_CONTEXT_MESSAGES) || DEFAULT_MAX_CONTEXT_MESSAGES;
    ...
    for (let i = history.length - 1; i >= 0; i--) {
      ...
      if (
        truncated.length > 0 &&
        (truncated.length >= MAX_CONTEXT_MESSAGES || tokenCount + msgTokens > MAX_ESTIMATED_TOKENS)
      ) {
