...
export class Server {
  ...
  private setupCoreRoutes(): void {
    this.app.get("/api/health", async (_req: Request, res: Response) => {
      const queueHealth = this.options.getQueueHealth ? await this.options.getQueueHealth() : null;
    ...
    this.app.post("/api/admin/restart", requireLocalhost, async (_req: Request, res: Response) => {
      const isWindowsManaged =
        process.platform === "win32" && process.env.CLAUDE_MEM_MANAGED === "true" && process.send;
    ...
    this.app.get("/api/admin/doctor", requireLocalhost, (_req: Request, res: Response) => {
      ...
      const envClean = !Object.keys(process.env).some(
        (key) => ENV_EXACT_MATCHES.has(key) || ENV_PREFIXES.some((prefix) => key.startsWith(prefix))
      );
