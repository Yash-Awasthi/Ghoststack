...
| 2   | `plugin/hooks/codex-hooks.json`                                                                                                                                                                           | 10, 15, 20, 32, 44, 56, 67 (every hook command)                                                   | Codex _should_ inject env → sh expands. Adds extra PATH-resolution prelude.                                                                                                                                                                                                       | 5 hook events (no `shell` field; sh assumed).                                                  |
...
| `plugin/hooks/codex-hooks.json`               | Codex CLI hook runner          | Codex _should_ inject env; sh expands                   | sh (no `shell` field) |
...
1. **Claude Code hook spec.** Does Claude Code documentation say `CLAUDE_PLUGIN_ROOT` is _guaranteed_ to be set at hook spawn time? Or only when the hook is loaded from a plugin (vs. a user-level hook)? Source: https://docs.claude.com/claude-code/ — find the hook contract page.
...
**What to implement:** A single TypeScript module that emits the canonical defensive shell prelude and the hook/MCP `command` strings. `scripts/build-hooks.js` calls it to _generate_ `plugin/hooks/hooks.json`, `plugin/hooks/codex-hooks.json`, `.mcp.json`, and `plugin/.mcp.json` from a single source of truth.
...
1. **Claude Code:** Is `CLAUDE_PLUGIN_ROOT` _guaranteed_ to be set for hooks in plugin-loaded `hooks.json` files (vs. user-level `hooks.json`)? Source: Claude Code docs.
