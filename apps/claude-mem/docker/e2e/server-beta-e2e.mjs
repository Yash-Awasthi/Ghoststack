...
async function json(response) {
  ...
  try {
    ...
    throw new Error(
      `Invalid JSON response (${response.status}): ${text}\n${error instanceof Error ? error.message : String(error)}`
    );
  }
}
...
async function expectStatus(path, status, options = {}) {
  ...
  assert(
    ...
    `${path} expected HTTP ${status}, got ${response.status}: ${await response.text()}`
  );
}
...
async function assertInfoEndpoint() {
  ...
  assert(
    body.postgres?.initialized === true,
    `expected postgres.initialized=true, got ${JSON.stringify(body.postgres)}`
  );
  assert(
    body.boundaries?.queueManager?.status === "active",
    `expected queue manager active, got ${JSON.stringify(body.boundaries?.queueManager)}`
  );
}
...
async function phase1() {
  ...
  assert(
    ...
    `session create failed: ${sessionRes.response.status} ${JSON.stringify(sessionRes.body)}`
  );
