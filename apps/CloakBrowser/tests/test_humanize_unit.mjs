...
await test("default config resolves", async () => {
  ...
  if (!Array.isArray(cfg.initial_cursor_x) || cfg.initial_cursor_x.length !== 2)
    throw new Error("initial_cursor_x invalid");
  if (!Array.isArray(cfg.initial_cursor_y) || cfg.initial_cursor_y.length !== 2)
    throw new Error("initial_cursor_y invalid");
});
...
await test("page.click is humanized", async () => {
  ...
  if (
    !clickStr.includes("ensureCursorInit") &&
    !clickStr.includes("humanClickFn") &&
    !clickStr.includes("scrollToElement")
  ) {
