...
async function isInputElementHandle(stealth: StealthEval | null, el: ElementHandle): Promise<boolean> {
  if (stealth) {
    try {
...
function patchSingleElementHandle(
  ...
  (el as any).waitForSelector = async (
    selector: string,
    options?: {
      hidden?: boolean;
      timeout?: number;
...
function patchSingleFrame(
  ...
  (frame as any).waitForSelector = async (
    selector: string,
    options?: {
      hidden?: boolean;
      timeout?: number;
