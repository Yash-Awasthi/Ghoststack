...
export async function humanClick(raw: RawMouse, isInput: boolean, cfg: HumanConfig): Promise<void> {
  const aimDelay = isInput ? randRange(cfg.click_aim_delay_input) : randRange(cfg.click_aim_delay_button);
  await sleep(aimDelay);

  const holdTime = isInput ? randRange(cfg.click_hold_input) : randRange(cfg.click_hold_button);
  await raw.down();
...
export function humanIdle(raw: RawMouse, cx: number, cy: number, cfg: HumanConfig): Promise<void>;
export function humanIdle(raw: RawMouse, seconds: number, cx: number, cy: number, cfg: HumanConfig): Promise<void>;
