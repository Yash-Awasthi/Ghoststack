...
<Note>
  ...
  Proxy`](/v2/servers/auth/oidc-proxy) for automatic configuration. OIDC Proxy extends the OAuth proxy to automatically
...
<ParamField body="token_verifier" type="TokenVerifier" required>
  A [`TokenVerifier`](/v2/servers/auth/token-verification) instance to validate the provider's tokens
</ParamField>
