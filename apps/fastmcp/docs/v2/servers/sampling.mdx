...
<Note>
  When you pass `result_type`, `sample()` automatically creates a `final_response` tool that the LLM calls to provide
  its response. If validation fails, the error is sent back to the LLM for retry. This automatic handling only applies
  to `sample()`—with `sample_step()`, you must manage structured output yourself.
</Note>
