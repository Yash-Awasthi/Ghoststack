...
describe.skipIf(SKIP_INTEGRATION_TESTS)(
  'Advisory Lock Integration Tests (Real DB)',
  () => {
    beforeAll(async () => {
      // Create test database connection
    ...
    describe('Concurrent credit consumption for same user', () => {
      it('should serialize concurrent consume operations and prevent race conditions', async () => {
        ...
        const finalGrant = await db.query.creditLedger.findFirst({
          where: eq(
            schema.creditLedger.operation_id,
            'concurrent-consume-grant',
          ),
        })
        ...
        console.log(
          'Concurrent consumption timings:',
          sortedResults.map((r) => ({
            consumed: r.consumed,
        ...
        console.log(
          `Total elapsed time: ${totalElapsed}ms (expected >${singleOpTime * 2}ms for serialization)`,
        )
    ...
    describe('Concurrent operations for different users', () => {
      it('should allow parallel execution for different users (no blocking)', async () => {
        ...
        console.log(
          `Parallel execution total time: ${totalTime}ms (expected ~${delayMs}ms for parallel)`,
        )
        ...
        const overlap =
          Math.min(result1.endTime, result2.endTime) -
          Math.max(result1.startTime, result2.startTime)
    ...
    describe('Mixed grant and consume operations', () => {
      it('should serialize grant and consume operations for the same user', async () => {
        ...
        const wasGrantFirst =
          grantResult.endTime <= consumeResult.startTime + 10
        const wasConsumeFirst =
          consumeResult.endTime <= grantResult.startTime + 10
        console.log(
          `Grant first: ${wasGrantFirst}, Consume first: ${wasConsumeFirst}`,
        )
      })
    })
          ...
          withAdvisoryLockTransaction({
            callback: async (tx) => {
              ...
              await tx
                ...
                .set({ balance: 50 })
                .where(
                  eq(schema.creditLedger.operation_id, 'rollback-test-grant'),
                )
    ...
    describe('Observability thresholds', () => {
      ...
      it('should not emit WARN logs for moderate contention (< 3s wait)', async () => {
        ...
        const warnLogs = logs.filter((l) => l.level === 'warn')
        console.log(
          `Contention test: ${warnLogs.length} warn logs (expected 0 for < 3s waits)`,
        )
        expect(warnLogs).toHaveLength(0)
      })
    ...
    describe('Hash collision resistance', () => {
      it('should use different lock hashes for user vs org with same ID', async () => {
        ...
        console.log(
          `User/Org parallel execution: ${totalTime}ms (expected ~${delayMs}ms for parallel)`,
        )
        expect(totalTime).toBeLessThan(delayMs * 1.8)
      })
    })
