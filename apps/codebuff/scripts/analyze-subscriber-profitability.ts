...
async function analyzeSubscriberProfitability() {
  ...
  console.log(
    `  Lookback: ${lookbackDays} days (since ${lookbackDate.toISOString().split('T')[0]})`,
  )
  console.log(
    `  Cost per credit: $${COST_PER_CREDIT.toFixed(6)} (PROFIT_MARGIN=${PROFIT_MARGIN})`,
  )
  ...
  try {
    ...
    console.log(
      `Found ${activeSubscribers.length} active subscribers (${activeSubscribers.length - filteredSubscribers.length} excluded)\n`,
    )
    ...
    for (const [tierPrice, subscribers] of [...tierGroups.entries()].sort(
      (a, b) => a[0] - b[0],
    )) {
      const tierConfig =
        SUBSCRIPTION_TIERS[tierPrice as keyof typeof SUBSCRIPTION_TIERS]
      if (!tierConfig) {
        console.log(
          `Unknown tier: $${tierPrice} (${subscribers.length} subscribers) — skipping`,
        )
        continue
      }
      ...
      const subscriberData = subscribers.map((sub) => {
        ...
        const projectedMonthlyProfit =
          tierConfig.monthlyPrice - projectedMonthlyCost
        const utilization =
          tierConfig.weeklyCreditsLimit > 0
        ...
        return {
          ...
          projectedMonthlyProfit:
            Math.round(projectedMonthlyProfit * 100) / 100,
      ...
      const weeklyCreditsArr = subscriberData
        .map((s) => s.weeklyCredits)
        .sort((a, b) => a - b)
      const avgWeeklyCredits =
        weeklyCreditsArr.reduce((a, b) => a + b, 0) /
        (weeklyCreditsArr.length || 1)
      const medianWeeklyCredits =
        weeklyCreditsArr.length > 0
          ? weeklyCreditsArr[Math.floor(weeklyCreditsArr.length / 2)]
          : 0
      const maxWeeklyCredits =
        weeklyCreditsArr.length > 0
      ...
      const analysis: TierAnalysis = {
        ...
        projectedMonthlyProfit:
          Math.round((tierConfig.monthlyPrice - projectedMonthlyCost) * 100) /
          100,
    ...
    console.log(`${'─'.repeat(80)}`)
    console.log(
      `  TIER SUMMARY (projected from ${lookbackDays}-day usage → monthly)`,
    )
    console.log(`${'─'.repeat(80)}\n`)
    ...
    for (const t of tierAnalyses) {
      ...
      console.log(
        `  ┌─ $${t.tier}/mo Tier (${t.subscriberCount} subscriber${t.subscriberCount !== 1 ? 's' : ''})`,
      )
      console.log(
        `  │  Weekly limit: ${t.weeklyLimit.toLocaleString()} credits`,
      )
      console.log(
        `  │  Break-even: ${t.breakEvenCreditsPerMonth.toLocaleString()} credits/mo (${((t.breakEvenCreditsPerMonth / maxMonthlyCredits) * 100).toFixed(1)}% utilization)`,
      )
      console.log(
        `  │  Max monthly cost: $${maxMonthlyCost.toFixed(2)} (at 100% utilization)`,
      )
      console.log(`  │`)
      console.log(
        `  │  Avg weekly usage:    ${t.avgWeeklyCredits.toLocaleString()} credits (${t.avgUtilization}% of limit)`,
      )
      console.log(
        `  │  Median weekly usage: ${t.medianWeeklyCredits.toLocaleString()} credits`,
      )
      console.log(
        `  │  Max weekly usage:    ${t.maxWeeklyCredits.toLocaleString()} credits`,
      )
      console.log(`  │`)
      console.log(
        `  │  Projected avg monthly cost:   $${t.projectedMonthlyCost.toFixed(2)}`,
      )
      console.log(
        `  │  ${profitIcon} Projected avg monthly profit: $${t.projectedMonthlyProfit.toFixed(2)} per subscriber`,
      )
      console.log(
        `  │  Total tier revenue: $${t.monthlyRevenue.toLocaleString()}/mo`,
      )
      ...
      console.log(
        `  │  ${tierProfitIcon} Total tier profit:   $${totalTierProfit.toFixed(2)}/mo`,
      )

      // Count profitable vs unprofitable subscribers
      const profitable = t.subscribers.filter(
        (s) => s.projectedMonthlyProfit >= 0,
      ).length
      const unprofitable = t.subscribers.length - profitable
      console.log(
        `  │  Profitable: ${profitable}  |  Unprofitable: ${unprofitable}`,
      )
      console.log(`  │`)
      ...
      console.log(
        `  │  ${'Email'.padEnd(35)} ${'Wk Credits'.padStart(12)} ${'Util %'.padStart(8)} ${'Mo Profit'.padStart(12)}`,
      )
      console.log(`  │  ${'─'.repeat(67)}`)
      for (const s of t.subscribers) {
        const icon = s.projectedMonthlyProfit >= 0 ? '✅' : '❌'
        const emailTrunc =
          s.email.length > 33 ? s.email.slice(0, 30) + '...' : s.email
        console.log(
    ...
    const totalSubscribers = tierAnalyses.reduce(
      (s, t) => s + t.subscriberCount,
      0,
    )
    ...
    const totalProjectedCost = tierAnalyses.reduce((s, t) => {
      return (
        s +
        t.subscribers.reduce(
          (sum, sub) =>
            sum + sub.weeklyCredits * WEEKS_PER_MONTH * COST_PER_CREDIT,
          0,
        )
      )
    }, 0)
    ...
    const profitableCount = tierAnalyses.reduce(
      (s, t) =>
        s +
        t.subscribers.filter((sub) => sub.projectedMonthlyProfit >= 0).length,
      0,
    )
    ...
    console.log(
      `  ${totalProfit >= 0 ? '✅' : '❌'} Net projected profit:  $${totalProfit.toFixed(2)}/mo`,
    )
    console.log(
      `  Profitable subscribers: ${profitableCount}/${totalSubscribers} (${((profitableCount / (totalSubscribers || 1)) * 100).toFixed(0)}%)`,
    )
    console.log(
      `  Unprofitable subscribers: ${unprofitableCount}/${totalSubscribers}`,
    )
    console.log(
      `  Avg profit margin: ${totalRevenue > 0 ? ((totalProfit / totalRevenue) * 100).toFixed(1) : 0}%`,
    )
    console.log()
  } catch (error) {
