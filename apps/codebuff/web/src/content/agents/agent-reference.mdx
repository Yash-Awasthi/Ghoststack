...
<MarkdownTable>
  | Yield Value | What It Does | | --- | --- | | Tool call object | Execute a
  tool and get its result | | 'STEP' | Run one LLM generation step | |
  'STEP_ALL' | Let the agent run until completion | | return | End the agent's
  turn immediately |
</MarkdownTable>
...
const definition: AgentDefinition = {
  ...
  spawnerPrompt:
    'Spawn this agent for writing documentation, API docs, or user guides',
  model: 'anthropic/claude-sonnet-4.5',
  outputMode: 'last_message',
  includeMessageHistory: true,
  ...
  instructionsPrompt:
    "Create documentation based on the user's request. Research existing code first.",
  stepPrompt:
