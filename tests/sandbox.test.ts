...
describe("Milestone 1: Filesystem Sandbox & Path Isolation", () => {
  ...
  it("should block path traversal attempts violating sandbox limits", async () => {
    ...
    await expect(sandbox.writeFile(illegalFile, "traversal content")).rejects.toThrow(
      "Sandbox Write violation"
    );

    await expect(sandbox.readFile(illegalFile)).rejects.toThrow(
      "Sandbox Read violation"
    );
  });
  ...
  it("should enforce write volume ceiling constraint limits", async () => {
    ...
    await expect(sandbox.writeFile(file2, "B".repeat(50))).rejects.toThrow(
      "Sandbox Write violation"
    );
  });
