...
export class PlanningEngine implements IPlanningEngine {
  async generatePlan(objective: string, context?: any): Promise<ICognitiveTrace> {
    ...
    if (normObj.includes('ingestion') || normObj.includes('scraper')) {
      ...
      synthesisResults.push({
        ...
        dependencies: [],
        priority: 'high',
        governanceMetadata: { dangerous: true, costEstimate: 0.00, resourceScope: 'aws:iam' }
      });
      ...
      synthesisResults.push({
        ...
        priority: 'high',
        governanceMetadata: { dangerous: false, costEstimate: 0.10, resourceScope: 'aws:s3' }
      });
