...
export type FastMcpHostOptions = {
  repoRoot: string;
  apiUrl?: string;
  mcpPort?: number;
...
/**
 * Manages the optional external FastMCP Python server (composite federation MCP).
 */
export class FastMcpHost {
  ...
  constructor(options: FastMcpHostOptions) {
  ...
  async start(): Promise<void> {
    ...
    if (!fs.existsSync(this.scriptPath)) {
      throw new Error(`FastMCP server script not found: ${this.scriptPath}`);
    }
    ...
    this.proc.stdout?.on("data", (d) => process.stdout.write(`[fastmcp] ${d}`));
    this.proc.stderr?.on("data", (d) => process.stderr.write(`[fastmcp] ${d}`));
    ...
    return new Promise((resolve, reject) => {
      const tick = async () => {
        ...
        if (Date.now() - started > timeoutMs) {
          reject(new Error(`FastMCP server did not start on port ${port} within ${timeoutMs}ms`));
          return;
        }
        setTimeout(tick, 400);
      };
