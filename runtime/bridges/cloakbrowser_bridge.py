"""
Stealth browser execution bridge.

Wraps the CloakBrowser Playwright extension to provide stealth Chromium
automation that bypasses bot-detection systems (Cloudflare, FingerprintJS,
reCAPTCHA v3). Exposed as a local FastAPI server on port 7701.
...
# ── Conditional import: graceful degradation when cloakbrowser not installed ──
try:
...
async def _fetch_with_cloak(req: BrowseRequest, capture_screenshot: bool = False) -> BrowseResponse:
    """Use CloakBrowser (patched Chromium) for stealth navigation."""
    import asyncio
...
async def _interact_with_cloak(req: InteractRequest) -> BrowseResponse:
    """Navigate and execute an action sequence using CloakBrowser."""
    async with await cb_launch_async(
        headless=req.headless,
        humanize=req.humanize,
        proxy=req.proxy,
...
@app.get("/health")
async def health():
    return {"status": "ok", "cloak_available": _CLOAK_AVAILABLE}
...
async def browse(req: BrowseRequest):
    if not _CLOAK_AVAILABLE:
        raise HTTPException(
            status_code=503,
            detail="Stealth browser unavailable: cloakbrowser not installed. Run: pip install cloakbrowser"
        )
    try:
