...
Wraps the airllm library which runs large language models (up to 70B+)
on consumer GPUs via layer-by-layer sharded inference. No quantization
required — achieves 70B inference on 4GB VRAM.
...
async def generate(req: GenerateRequest):
    if not _AIRLLM_AVAILABLE:
        raise HTTPException(
            status_code=503,
            detail="airllm not installed. Run: pip install airllm"
        )
    try:
...
async def chat(req: ChatRequest):
    if not _AIRLLM_AVAILABLE:
        raise HTTPException(status_code=503, detail="airllm not installed")
    try:
        model = _load_model(req.model, req.compression)
