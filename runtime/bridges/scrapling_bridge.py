"""
Web scraping execution bridge.

Wraps the Scrapling library — an adaptive, anti-detection scraping engine
with multiple fetcher backends: HTTP (curl_cffi), stealth Chromium, and
regular Chromium for JavaScript-heavy targets.
...
def _extract_selectors(response: Any, selectors: list[str]) -> dict[str, str]:
    """Extract CSS selector results from a Scrapling Response."""
    extracted: dict[str, str] = {}
    if not selectors:
        return extracted
    try:
...
async def fetch(req: FetchRequest):
    if not _SCRAPLING_AVAILABLE:
        raise HTTPException(
            status_code=503,
            detail="Scrapling not installed. Run: pip install scrapling"
        )
    try:
        kwargs: dict[str, Any] = {
            "timeout": req.timeout,
...
async def fetch_stealth(req: FetchRequest):
    if not _SCRAPLING_AVAILABLE:
        raise HTTPException(status_code=503, detail="Scrapling not installed")
    try:
        kwargs: dict[str, Any] = {
