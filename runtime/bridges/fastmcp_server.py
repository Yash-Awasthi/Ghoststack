"""
MCP server bridge using FastMCP.

Replaces the manual TypeScript GhostStack MCP bridge implementation with
a clean decorator-based FastMCP server. GhostStack's TypeScript MCP client
continues to use the same protocol — only the server implementation changes.

Exposes GhostStack capabilities as MCP tools:
...
if _FASTMCP_AVAILABLE:
    ...
    # FastMCP not installed — serve a minimal health endpoint
    from fastapi import FastAPI
    app = FastAPI(title="ghoststack-mcp-bridge-fallback")

    @app.get("/health")
    async def health():
        return {"status": "ok", "fastmcp_available": False}
...
if __name__ == "__main__":
    ...
    if _FASTMCP_AVAILABLE and args.transport != "streamable-http":
        # Run via FastMCP's native runner for stdio/sse transports
